...
### 3. 何时写、查阅流程与决策所有权（Owning Note）

1. **修改前查阅（Pre-edit Review）**：在对交易核心、连接器逻辑、账户边界或数据流进行非平凡修改前，必须先在 `.agents/notes/implemented/` 检索对应的历史笔记（Owning Note），重点审阅其 `Alternatives considered` 与 `Consequences`，避免重犯历史错误或推翻既定风控/架构约束。
2. **决策所有权（The Owning Note Rule）**：
   - 每个非平凡变更（行为、架构、跨包契约、流程工具、测试策略、持久化/传输/配置格式）**必须在同一变更中**新增或更新记录。
   - **更新已拥有该决策的旧记录即满足要求，严禁新建重复记录**。纯代码重命名、路径迁移或参数微调时，原地更新已有 Note 的事实陈述。
   - 纯机械局部修改豁免。仅当无既有 Note 覆盖或引入全新/相反决策时，才新建记录并建立交叉链接（Cross-link）。
3. **交付态写事实（Keep Current With Shipped Reality）**：`implemented/` 下只写现在时客观事实，规格腔标题（Proposal / Plan / Acceptance criteria）禁入；代码变动时笔记事实同步保鲜。遵循「一个事实只有一个家（One home per fact）」。
...
4. **门禁边界验证**：修复后重跑受影响检查；提交/推送/发布边界运行仓库规定的完整门禁（`pnpm -r test` 与 `pnpm -r build`），不要求每次局部修改后全量重跑。

---

## 第四部分：效能治理与模型分级契约（Pareto Model Tiering & Anti-Thrashing）

1. **子 Agent 模型选择**：按任务难度、错误成本与时延在运行时可用模型中选择，不写死供应商档位别名；架构设计与疑难根因分析用高推理能力，机械检索与同步可用轻量模型。
2. **按需工具挂载（Tool Scoping）**：避免无脑全量注入 MCP / Tool Schema；根据当前 Skill 和 Subagent 角色最小化暴露工具集。
3. **反震荡**：同一思路重试若无新证据（同一文件或报错反复、改动往复撤销），立即停止原路径，归纳证据、换有依据的方向；只有缺信息、缺授权或存在重大取舍时才向用户提问，固定次数本身不是提问理由。
