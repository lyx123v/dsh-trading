...
2. **最小可用市场（数据面切片）**：
   - `connector-hithink` 新增 `HiThinkFuturesMarketDataService`：getTicker = 分时末点优先、回落日K收盘，日K与分时皆空时抛 `TRADING_UNSUPPORTED_SYMBOL`（2026-09-14 修，见 [中金所 8888 加权无行情](../bug-fix/2026-09-14-hithink-futures-weighted-no-data.md)）；getKlines = 1d 直连、分钟周期由当日分时聚合（仅当日，不冒充历史）、1w/1M 日K聚合；listInstruments = 带 query 走 `/api/meta/tickers/search?asset_type=futures`、无 query 走代码表（过滤 last_trade_date 已过期合约，并剔除上游无行情的中金所 8888 加权码）；futures-dataplane 注册 `(futures, hithink)`（subscribeTicker 10s 轮询）。
