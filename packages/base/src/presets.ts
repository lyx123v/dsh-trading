...
export function composePresets(contributions: readonly MarketContribution[]) {
  ...
  return PRESET_IDS.map(id => {
    ...
    const delegation = id === 'master'
      ? `- id: dsh-trading-master-jobs\n  name: '@deepseek-ai/dsh-tool-jobs'\n\n` + DELEGATES.map(d => `- id: ${d.id}\n  name: '@deepseek-ai/dsh-tool-subagent'\n  config:\n    provider: fork\n    toolName: ${d.tool}\n    backgroundMode: one-shot\n    persona: ${JSON.stringify(`${ROLES[d.role].text} ${DOCTRINE} ${EVIDENCE} ${VERDICT} ${DELEGATE_RULE}`)}\n`).join('\n')
      : ''
  ...
  if (results.every(result => result.skipped.length === 0)) {
    for (const market of MARKETS) {
      ...
      try {
        ...
        const archived = await Promise.all(FILES.map(file => readOwnedFile(join(backup, file))))
        if (archived.every(text => text === null)) {
          await rename(dir, backup)
        } else if (FILES.every((_, i) => archived[i] === contents[i])) {
          // 同一份受管默认已归档，却被旧版市场安装器重新写回名册；备份名被占时旧逻辑
          // 会永久跳过退役，废弃预设就此常驻。此处只删名册副本，归档与用户文件不动。
          await rm(dir, { recursive: true })
        } else {
          // 备份名下是另一版本的旧默认：按内容哈希另名归档，备份本身绝不覆盖。
          // 同名即同内容（哈希命中），说明该版本已归档，只需清掉名册副本。
          const alt = join(backupRoot, `${id}.${hash(contents.join('\n'))}`)
          if (await readOwnedFile(join(alt, FILES[0])) === null) await rename(dir, alt)
