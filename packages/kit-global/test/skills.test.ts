/**
 * kit-global skill provider：名册、按名分发与白名单收窄
 * （与 kit-futures/test 同口径；共享技能目录与 base role-skill 白名单必须对齐，
...
describe('kit-global skill provider', () => {
  ...
  it('get 按名分发：各个 skill 的 content 都真实可读', async () => {
    for (const name of EXPECTED) {
      const skill = await provider.get({ name, provider: 'dsh-trading-global' } as never)
  ...
  it('get 未知名字回落 risk-checklist（防御）', async () => {
    ...
    expect(skill.name).toBe('global-risk-checklist')
  })

  it('providerForSkills 按白名单收窄：空名单不暴露、名单外 get 拒绝、未知名 fail-fast、缺省全量', async () => {
    const scoped = providerForSkills(['global-risk-checklist', 'trading-notes-setup'])
    expect((await scoped.list()).map((c) => c.name)).toEqual(['global-risk-checklist', 'trading-notes-setup'])
    ...
    await expect(empty.get({ name: 'global-risk-checklist' } as never)).rejects.toThrow('not in whitelist')
  ...
  it('base role-skill 白名单可完整解析（trader / instrument-researcher / risk-reviewer 三套）', async () => {
    for (const whitelist of [
      ['global-risk-checklist', 'trading-strategy-paradigms', 'indicator-authoring', 'trading-notes-setup'],
      ['knowledge-curation', 'trading-notes-setup'],
      ['global-risk-checklist', 'trading-notes-setup'],
    ]) {
